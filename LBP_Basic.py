...
def lbp_images(dataset, dir_path, wind, block, shift):
    ...
    for imgpdst_file in os.listdir(pdst_path):
        ...
        lbp_pdst = LBPbas_compute(img,wind,block,shift)                 #toma cada foto y extrae sus caracteristicas y
        try:                                                            #las anyade a la matriz
            images_mat = np.vstack((images_mat,lbp_pdst))
        except NameError:
            images_mat = lbp_pdst
    ...
    for imgback_file in os.listdir(back_path):
        ...
        lbp_back = LBPbas_compute(img,wind,block,shift)                 #toma cada foto y extrae sus caracteristicas y
        try:                                                            #las anyade a la matriz
            images_mat = np.vstack((images_mat,lbp_back))
        except NameError:
            images_mat = lbp_back
...
train_dat, train_class = lbp_images('train', p_path, wind_size, block_size, shift_size)

#Aplicacion del algoritmo para estudiar las caracteristicas de las imagenes de test
test_dat, test_class = lbp_images('test',p_path, wind_size, block_size, shift_size)
...
with open("lbp_class.pickle", "wb") as f:
    pickle.dump(cv_tune, f)

#Obtencion en array de los resultados de la busqueda de parametros por validacion cruzada
lbp_data = np.asarray(cv_tune.cv_results_['params'])
for i in range(0,len(lbp_data)):
    lbp_result_list = list(lbp_data[i].values())
    if len(lbp_result_list)<4:
        lbp_result_list.insert(1,"-")
    try:
        lbp_result_mat = np.vstack((lbp_result_mat, lbp_result_list))
    except NameError:
        lbp_result_mat = lbp_result_list
lbp_accuracy = cv_tune.cv_results_['mean_test_accuracy']
lbp_auc = cv_tune.cv_results_['mean_test_AUC']
lbp_f1 = cv_tune.cv_results_['mean_test_f1']
lbp_scores_sum = np.vstack((lbp_accuracy,lbp_auc,lbp_f1))
lbp_scores_mat = np.transpose(lbp_scores_sum)
lbp_latex_summary = np.hstack((lbp_result_mat, lbp_scores_mat))
ascii.write(lbp_latex_summary,format='latex',names=['C','Degree','$\gamma$','Kernel','Accuracy','AUC','F1'])
